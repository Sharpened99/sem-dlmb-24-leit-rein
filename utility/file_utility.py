...
class FileUtility(object):
    ...
    @staticmethod
    def create_fasta_file(file_address, corpus, label, num=True, type='protein'):
        seq_id_pairs = [('.'.join([str(idx + 1), label[idx]]) if num else label[idx], x) for idx, x in enumerate(corpus)]
